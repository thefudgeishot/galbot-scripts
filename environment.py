...
class IoaiNavEnv:
    ...
    def _setup_simulator(self, headless):
        ...
        config = PhysicsSimulatorConfig(
            mujoco_config=MujocoConfig(headless=headless,
                                       timestep=0.01) # run the simulation at 0.1s per step
        )
    ...
    def _init_pose_done(self):
        ...
        if (right_armState | self._is_joint_positions_reached(self.interface.right_arm, self.right_arm)):
            right_armState = True
        nameIndex = ["headState","legState","left_armState","right_armState"]
        itemIndex = [headState,legState,left_armState,right_armState]
        index = 0
        for item in itemIndex:
            print("States(" + str(nameIndex[index]) + "): " + str(item))
            index+=1 

        print("Left Arm: " + str(self.interface.left_arm.get_joint_positions()))
        print("Right Arm: " + str(self.interface.right_arm.get_joint_positions()))
